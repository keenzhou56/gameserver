...
// SetUserOnline ...
func (d *Dao) SetUserOnline(c context.Context, online uint64) (err error) {
	conn := d.redis.Get()
	key := keyServerOnline("user")
	_, err = conn.Do("SET", key, online)
	conn.Close()
	return
}

// GetUserOnline ...
func (d *Dao) GetUserOnline(c context.Context) (online uint64, err error) {
	conn := d.redis.Get()
	key := keyServerOnline("user")
	online, err = redis.Uint64(conn.Do("GET", key))
