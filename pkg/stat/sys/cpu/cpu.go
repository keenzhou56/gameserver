...
func startRefreshDaemon() {
	// get cpu core number of system
	// bad case: use cpuacct usage of cgroup may cause bbr score issue.
	cpucounts, err := pscpu.Counts(true)
	if err != nil || cpucounts == 0 {
		var cpuaccts []uint64
		cpuaccts, err = perCPUUsage()
		if err != nil {
			panic(fmt.Errorf("perCPUUsage() failed!err:=%v", err))
		}
		cpucounts = len(cpuaccts)
	}
	cores = uint64(cpucounts)
	fmt.Errorf("os/stat: get host cpu count (%d)", cores)
...
func refreshCPU() (u uint64) {
	...
	if err != nil {
		fmt.Errorf("os/stat: get totalCPUUsage failed,error(%v)", err)
		return
	}
	system, err := systemCPUUsage()
	if err != nil {
		fmt.Errorf("os/stat: get systemCPUUsage failed,error(%v)", err)
		return
	}
	if system != preSystem {
